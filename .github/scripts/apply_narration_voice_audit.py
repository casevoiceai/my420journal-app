...
new_prompt = r'''You are Eliza, the GameMaster of Weed Goblins. The deterministic engine decides what happened. Your job is to run that fact at the table so it feels continuous, specific, responsive, and human.
...
pattern = r"export const WEED_GOBLINS_SYSTEM_PROMPT = `.*?`\n\nfunction jsonResponse"
replacement = "export const WEED_GOBLINS_SYSTEM_PROMPT = `" + new_prompt + "`\n\nfunction jsonResponse"
worker, count = re.subn(pattern, replacement, worker, count=1, flags=re.S)
assert count == 1, 'system prompt block not found'
...
worker = worker.replace('max_tokens: 96,', 'max_tokens: 220,')
worker_path.write_text(worker)

validation_path = Path('src/features/games/weed-goblins/weedGoblinsNarrationValidation.js')
...
first_person_block = """  const narratorFrame = narratorFrameForFirstPerson(text, moment)
  if (!FIRST_PERSON_SIGNAL.test(narratorFrame)) {
    reasons.push('is not written in first person')
  }
"""
assert first_person_block in validation
validation = validation.replace(first_person_block, """  if (NARRATOR_OBSERVER_SIGNAL.test(text)) {
...
"""
assert active_blocks in validation
validation = validation.replace(active_blocks, '')

locked_opening = """  if (moment === 'scene-intro' && introKind === 'highlands-opening') {
...
"""
assert locked_opening in validation
validation = validation.replace(locked_opening, """  if (moment === 'scene-intro' && introKind === 'highlands-opening'
...
old_check = """  const advantageText = preview.advantage
...
"""
new_check = """  const advantageText = preview.advantage
...
"""
assert old_check in controller
controller = controller.replace(old_check, new_check)
