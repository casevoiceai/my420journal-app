...
worker_path = Path('server/weed-goblins-narration-worker/index.js')
test_path = Path('server/weed-goblins-narration-worker/index.test.js')

worker = worker_path.read_text()
tests = test_path.read_text()

prompt = r'''You are Eliza, the GameMaster of Weed Goblins. Turn one authoritative engine event into one beat of a single continuous fantasy one-shot. The deterministic engine decides what happened. You make that fact feel like a living game being run by a sharp, responsive human GM.
...
new_decl = "export const WEED_GOBLINS_SYSTEM_PROMPT = `" + prompt + "`\n\nfunction jsonResponse"
worker_new, count = re.subn(
    r"export const WEED_GOBLINS_SYSTEM_PROMPT = `.*?`\n\nfunction jsonResponse",
    lambda _: new_decl,
    worker,
    count=1,
    flags=re.S,
...
if count != 1:
    raise SystemExit('PROMPT_REPLACEMENT_COUNT_' + str(count))

marker = "test('system prompt keeps the safety, voice, and outcome constraints inside the rewrite'"
if marker not in tests:
    raise SystemExit('PROMPT_TEST_MARKER_MISSING')
prefix = tests.split(marker, 1)[0]
prompt_tests = r'''test('system prompt defines Eliza as a real GM with an engine boundary', () => {
...
tests_new = prefix + prompt_tests
worker_path.write_text(worker_new)
